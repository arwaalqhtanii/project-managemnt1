...
useEffect(() => {
    const fetchCounts = async () => {
        try {
            const response = await axios.get('http://localhost:5040/projects/projects/student/counts', {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            setCounts(response.data.counts); // Assuming response data contains counts
        } catch (error) {
    ...
    <div className="min-h-screen bg-gray-100 font-sans">
      ...
      <section id="stats" className="bg-white py-16 px-12">
        <div className="container mx-auto flex justify-between items-center flex-col">
          <h2 className="text-4xl font-bold text-center text-gray-800 mb-20">Your Project Statistics</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8  lg:w-[80%]">
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 transform hover:scale-105 flex flex-col items-center">
              ...
              <p className="text-4xl font-bold mt-6 text-gray-800">{counts.submitted}</p>
